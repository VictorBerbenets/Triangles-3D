...
line_t::line_t(const point_t &pt1, const point_t &pt2)
    : dir_coords_{pt1, pt2}, point_{pt1} {
  if (pt1 == pt2) {
...
line_t::line_t(const vector_t &dirr_vec, const point_t &pt)
    : dir_coords_{dirr_vec}, point_{pt} {}
...
point_t line_t::get_intersec_point(const line_t &other) const {
  ...
  std::array<pair, 3> dirs{pair{dir_coords_[0], 0}, pair{dir_coords_[1], 1},
                           pair{dir_coords_[2], 2}};
  ...
  for (std::size_t id = 1; id < 3; ++id) {
    ...
    double expr = other.dir_coords_[neigh_id] * val -
                  dirs[neigh_id].first * other.dir_coords_[nzero_it->second];
    if (!is_zero(expr)) {
      coeff = ((dirs[neigh_id].first * diffs[nzero_it->second]) / val -
               diffs[neigh_id]) *
              (val / expr);
    }
  }
...
bool line_t::is_valid() const {
  return point_.is_valid() && dir_coords_.is_valid();
}
...
segment_t::segment_t(const point_t &pt) : pt1_{pt}, pt2_{pt} {};

point_t segment_t::center() const noexcept {
  return {(pt1_.x_ + pt2_.x_) / 2, (pt1_.y_ + pt2_.y_) / 2,
          (pt1_.z_ + pt2_.z_) / 2};
}
...
bool segment_t::is_valid() const { return pt1_.is_valid() && pt2_.is_valid(); }
...
bool segment_t::is_degenerated() const { return pt1_ == pt2_; }
...
void segment_t::set_ends(const point_t &pt) { pt1_ = pt2_ = pt; }
