...
vector_t::vector_t(value_type coord) : coords_{coord, coord, coord} {}

vector_t::vector_t() : vector_t(NAN) {}

bool vector_t::operator==(const vector_t &rhs) const noexcept {
  return are_equal(coords_[0], rhs[0]) && are_equal(coords_[1], rhs[1]) &&
...
vector_t &vector_t::operator/=(value_type coeff) {
  if (is_zero(coeff)) {
    throw std::invalid_argument{
        "Attempt to divide by 0 in vector's operator/=\n"};
  }
  for (auto &val : coords_) {
    val /= coeff;
  }
...
vector_t::value_type vector_t::get_module() const {
  return std::sqrt(coords_[0] * coords_[0] + coords_[1] * coords_[1] +
                   coords_[2] * coords_[2]);
}

vector_t::array vector_t::coords() const noexcept { return coords_; }

bool vector_t::is_null() const {
  return is_zero(coords_[0]) && is_zero(coords_[1]) && is_zero(coords_[2]);
}

bool vector_t::is_valid() const {
  return !std::isnan(coords_[0]) && !std::isnan(coords_[1]) &&
         !std::isnan(coords_[2]);
}

vector_t::size_type vector_t::size() const noexcept { return coords_.size(); }
