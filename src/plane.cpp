...
plane_t::plane_t(const point_t &pt1, const point_t &pt2, const point_t &pt3) {
  ...
  D_ = -(pt1.y_ * normal_coords_[1] + pt1.x_ * normal_coords_[0] +
         pt1.z_ * normal_coords_[2]);
}
...
bool plane_t::contains(const point_t &pt) const {
  return is_zero(normal_coords_[0] * pt.x_ + normal_coords_[1] * pt.y_ +
                 normal_coords_[2] * pt.z_ + D_);
}
...
point_t plane_t::get_plane_point() const {
  ...
  std::array<solve_pair, 3> equation = {
      solve_pair{normal_coords_[0], Coords::X},
      solve_pair{normal_coords_[1], Coords::Y},
      solve_pair{normal_coords_[2], Coords::Z}};
  auto not_zero_coeff =
      std::find_if(equation.begin(), equation.end(),
...
static plane_t::minor_pair
find_not_zero_minor(const plane_t::plane_coeffs &coeffs1,
                    const plane_t::plane_coeffs &coeffs2) {
  using size_type = plane_t::size_type;

  for (size_type index = 0; index < 3; ++index) {
    size_type next_index = (index + 1) % 3;
    double det = determ(coeffs1[index], coeffs1[next_index], coeffs2[index],
                        coeffs2[next_index]);
    if (!is_zero(det)) {
...
point_t plane_t::get_intersec_point(const plane_t &rhs) const {
  ...
  plane_coeffs coeffs1{normal_coords_[0], normal_coords_[1], normal_coords_[2],
                       D_};
  plane_coeffs coeffs2{rhs.normal_coords_[0], rhs.normal_coords_[1],
                       rhs.normal_coords_[2], rhs.D_};
  ...
  double det_colum1 = determ(
      -coeffs1[3] - ARBITRARY_VALUE * coeffs1[arbit_col], coeffs1[colum2],
      -coeffs2[3] - ARBITRARY_VALUE * coeffs2[arbit_col], coeffs2[colum2]);
  double det_colum2 = determ(
      coeffs1[colum1], -coeffs1[3] - ARBITRARY_VALUE * coeffs1[arbit_col],
  ...
  return {0 == arbit_col ? ARBITRARY_VALUE
                         : determ_and_id[0].first / not_zero_minor,
          1 == arbit_col ? ARBITRARY_VALUE
                         : determ_and_id[1].first / not_zero_minor,
          2 == arbit_col ? ARBITRARY_VALUE
...
void plane_t::print() const {
  std::cout << normal_coords_[0] << "x + " << normal_coords_[1] << "y + "
            << normal_coords_[2] << "z + " << D_ << " = 0\n";
};
