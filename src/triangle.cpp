...
bool mul_vect_products(const triangle_t::pts_vector &tria,
                       const point_t &comp_pt) {
...
void find_intsec_points(std::vector<point_t> &intsec_points,
                        line_t &intsec_line, const point_t &pt1,
                        const point_t &pt2) {
  auto push_back_if = [&intsec_points](const point_t &pt) {
    if (std::find(intsec_points.begin(), intsec_points.end(), pt) ==
        intsec_points.end()) {
...
bool init_segment(line_t &intsec_line, const triangle_t &tria,
                  segment_t &segm) {
  std::vector<point_t> intsec_points{};
  for (int index = 0; index < 3; ++index) {
    find_intsec_points(intsec_points, intsec_line, tria[index],
                       tria[(index + 1) % 3]);
...
bool different_intersection(const tria_plane &pair1, const tria_plane &pair2) {
  ...
  if (!init_segment(intsec_line, pair1.first, tr1_segment)) {
    return false;
  }
  segment_t tr2_segment{};
  if (!init_segment(intsec_line, pair2.first, tr2_segment)) {
    return false;
  }
...
/*-------------------------------------triangles
 * interface-------------------------------------------*/
triangle_t::triangle_t(const point_t &pt1, const point_t &pt2,
                       const point_t &pt3)
    : vertices_{pt1, pt2, pt3} {};

triangle_t::triangle_t(double val1, double val2, double val3, double val4,
                       double val5, double val6, double val7, double val8,
                       double val9)
    : vertices_{point_t{val1, val2, val3}, point_t{val4, val5, val6},
...
bool triangle_t::is_intersecting(const triangle_t &rhs) const {
  ...
  } else if (pair1.second.contains(
                 pair2.second.get_plane_point())) { // if planes are parallel
                                                    // and one plane contains
    return pair1.first.same_intersection(
        pair2.first); // a point of another plane --> they are the same
  }
  return false;
}
...
bool triangle_t::same_intersection(const triangle_t &rhs) const {
  ...
  if (lhs.check_inside_point(rhs)) {
    return true;
  }
  if (rhs.check_inside_point(lhs)) {
    return true;
  }
  // if at least one segment of one tria intersec another tria's segment
  if (lhs.check_segments_intersection(rhs)) {
    return true;
  };
  return false;
}
...
bool triangle_t::check_inside_point(const triangle_t &rhs) const {
  ...
  for (size_type index1 = 0; index1 < POINTS_TO_COMPARE; ++index1) {
    ...
    for (size_type index2 = 0; index2 < POINTS_TO_COMPARE; ++index2) {
      if (mul_vect_products({lhs[index2], lhs[(index2 + 1) % POINTS_TO_COMPARE],
...
bool triangle_t::check_segments_intersection(const triangle_t &rhs) const {
  ...
  for (size_type index1 = 0; index1 < POINTS_TO_COMPARE; ++index1) {
    for (size_type index2 = 0; index2 < POINTS_TO_COMPARE; ++index2) {
      if (lhs_segms[index1].is_intersect(rhs_segms[index2])) {
        return true;
      }
    }
  }
...
point_t &triangle_t::operator[](std::size_t index) { return vertices_[index]; }
