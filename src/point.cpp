...
point_t::point_t() : x_{NAN}, y_{NAN}, z_{NAN} {};

point_t::coords point_t::get_coords() const noexcept { return {x_, y_, z_}; }
...
bool point_t::operator==(const point_t &other) const {
  return are_equal(x_, other.x_) && are_equal(y_, other.y_) &&
...
point_t &point_t::operator/=(double coeff) {
  if (is_zero(coeff)) {
    throw std::invalid_argument{
        "Attempt to divide by zero in point's operator/\n"};
  }
  x_ /= coeff;
  y_ /= coeff;
  z_ /= coeff;
...
point_t operator/(double coeff, const point_t &rhs) { return rhs / coeff; }
