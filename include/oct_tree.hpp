...
protected:
  ...
  static constexpr double MAX_HLF_SIDE =
      std::pow(2, 32); // 2^32 - half side's length
...
public:
  ...
  Node(const point_t &center, double hlf_side, const Indicator &id,
       size_type deep);
  ~Node() = default;
...
private:
  ...
  void diving_into_tree(const Node &node, Collector &col,
                        collision_list &collision_list) const;
  template <typename Collector>
  void check_collis_inside_node(Collector &col, const collision_list &ls) const;
  template <typename Collector>
  void check_collis_between_nodes(Collector &col, const collision_list &ls1,
                                  const collision_list &ls2) const;
...
template <typename Collector>
void OctTree::diving_into_tree(const Node &node, Collector &col,
                               collision_list &collision_list) const {
  ...
  collision_list.splice(collision_list.end(),
                        node_data); // adding collision triangles
...
template <typename Collector>
void OctTree::check_collis_inside_node(Collector &col,
                                       const collision_list &ls) const {
...
template <typename Collector>
void OctTree::check_collis_between_nodes(Collector &col,
                                         const collision_list &ls1,
                                         const collision_list &ls2) const {
